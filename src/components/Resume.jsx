...
function Info ({info, isSkill}) {
  return (
    isSkill === true
    ? <Skills skills={info} />
    : info.map((exp, index) => {
      return (
        <ResumeInfo key={index} info={{
          start: exp.start, 
          end: exp.end, 
          city: exp.location, 
...
export default function Resume ({layout, color, fonts, info, visible}) {
  ...
  const [personal, setPersonal] = useState(info.personal);
    return (
      visible === true
            ...
            <div className="infoContainer">
              <div className="experienceBox">
                <div className="cv-categories" style={{ background: invertedColor, color: color, borderStyle: "solid", borderColor: color, borderWidth: '2px', fontSize: '1.2rem'  }}>Profesional Experience</div>
                  <Info info={info.experience} isSkill={false}/>
              </div>
              <div className="educationBox">
                <div className="cv-categories" style={{ background: invertedColor, color: color, borderStyle: "solid", borderColor: color, borderWidth: '2px', fontSize: '1.2rem'  }}>Education</div>
                  <Info info={info.education} isSkill={false}/>
              </div>
              <div className="skillsBox">
                ...
                <Info info={info.skills} isSkill={true} />
              </div>
              </div>
          </aside>
        : <aside id={layout} style={{ fontFamily: fonts, fontSize: '1rem' }} className="bg-slate-100 shadow-xl">
            <div className="personalInfo p-6 h-28" style={{ background: color, color: invertedColor }}>
            </div>
          </aside>
    )
  }
